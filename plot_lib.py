...
img_path = os.path.join(my_path,"../img")
def set_default(figsize=(10, 10), dpi=100):
    plt.style.use(['seaborn-paper'])
#     plt.rc('axes', facecolor='white')
#     plt.rc('figure', facecolor='gray')
...
def plot_model(X, y, model, plot_points=False):
    model.cpu()
...
def plot_submodel(sub_prob,models,figsize=(10,10),dpi=100,caption=None,imgName=None,plot_points=False):
    sp = plt.figure(figsize=figsize,dpi=dpi)
    n=1
    for i in range(len(models)):
        ax = sp.add_subplot(1,4,n)
        if caption !=None:
            ax.set_title(caption[i])
        ax.set_yticks([])
        ax.set_xticks([])
        x_train, y_train = sub_prob[i]
        plot_model(x_train,y_train,models[i],plot_points)
        n+=1
    if imgName != None:
        sp.savefig(os.path.join(img_path,imgName+'.png'),dpi=100,)
    
def plot_subproblem(sub_prob,figsize=(10,10),dpi=100):
    sp = plt.figure(figsize=figsize,dpi=dpi)
    n=1
    for a in sub_prob:
        ax = sp.add_subplot(1,4,n)
